...
import { LayoutDashboard, History, Hammer, X } from 'lucide-react';
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const navLinks = [
    ...
    { name: 'Upgrades', path: '/upgrades', icon: <Hammer size={22} /> },
  ];
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-cardBg border-r border-white/5 z-[60] transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="p-8 flex flex-col h-full">
          <div className="flex justify-between items-center mb-10">
            <h1 className="text-2xl font-rajdhani font-black italic tracking-widest">ROMIL<span className="text-accent">CHAVDA</span></h1>
