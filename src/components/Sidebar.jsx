...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const navLinks = [
    ...
    { name: 'Widgets', path: '/widgets', icon: <LayoutGrid size={22} /> }, // Naya Widgets Page
  ];
        ...
        <nav className="space-y-3 flex-1">
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)}
              className={`flex items-center gap-4 p-4 rounded-2xl font-rajdhani font-bold text-lg transition-all ${location.pathname === link.path ? 'bg-accent text-black shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
              {link.icon} {link.name}
            </Link>
          ))}
        </nav>
